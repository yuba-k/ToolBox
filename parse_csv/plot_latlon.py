...
def draw_scatter(data,filename,goal_coordinates,saveFlag,displayFlag):
    ...
    if saveFlag:
        saved_name = os.path.split(filename.name)[1].split('.')[0]
...
def main():
    ...
    parser.add_argument("-o","--open",type=argparse.FileType("r"),help="解析対象のファイルのパス指定",required=True)
    ...
    if df.empty:
        ...
        return
    draw_scatter(data=df,filename=args.open,goal_coordinates=args.goal,saveFlag=args.save,displayFlag=args.display)
